  ...
  public function echoln($message, $colorizer_parameter = null, $colorize = true)
  {
    if ($colorize)
    {
      ...
      $message = preg_replace_callback(
        ...
        function ($m) {
          return $this->colorizer->colorize($m[1], 'ERROR');
        },
        $message
      );
      ...
      $message = preg_replace_callback(
        '/(?:^|\.)(ok *\d*)\b/',
        function ($m) {
          return $this->colorizer->colorize($m[1], 'INFO');
        },
        $message
      );
      ...
      $message = preg_replace_callback(
        '/"(.+?)"/',
        function ($m) {
          return $this->colorizer->colorize($m[1], 'PARAMETER');
        },
        $message
      );
      //
      ...
      $message = preg_replace_callback(
        '/(\->|\:\:)?([a-zA-Z0-9_]+?)\(\)/',
        function ($m) {
          return $this->colorizer->colorize($m[1] . $m[2] . '()', 'PARAMETER');
        },
        $message
      );
    }
